...
BEGIN
	...
	-- Se valida que minimamente exista mas de 1 hombre y una mujer para cumplir reglas de negocio sobre las
	-- combinaciones en las mesas
	IF (menQuantity > 1 AND womenQuantity > 1) THEN
        BEGIN
        	-- Recorrido incial (Se recorren primero los datos filtrados con menor cantidad de registros encontrados)
		...
		END;
        BEGIN
			-- Recorrer final ( Se realiza recorrido de la informacion restante ya sea de hombres o mujeres para 
			-- completar el par ordenado de hombres y mujeres en cada mesa)
        ...
        END;
	ELSE 
		-- Se inserta solo un registro ya que no es posible armar el grupo selecto de invitados
        INSERT INTO ResponseTable(clientCode, encrypt, company, balance)
